...
public class SwipeBackLayout extends FrameLayout {
    ...
    private boolean isOpenClickX=true;
...
//        setShadow(R.drawable.shadow_left);
    ...
    public void setOpenClickX(boolean openClickX) {
        isOpenClickX = openClickX;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        if (clickX == -1&&isOpenClickX) clickX = event.getX();
        if (event.getAction() == MotionEvent.ACTION_UP&&isOpenClickX) clickX = -1;
        if (clickX > mContentView.getWidth() * 0.9&&isOpenClickX) return false;
        if (!mEnable || mDisallowIntercept) return false;
        try {
